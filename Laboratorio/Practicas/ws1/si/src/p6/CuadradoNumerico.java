...
public class CuadradoNumerico
{
	...
	static boolean haySolucion = false; // tendra valor true si se ha encontrado una solucion (en esta version del algoritmo a computar nos interesa hacer la poda)
	...
	private static void cuadradoNumerico(int nivel, int indiceColumna)
	{
		...
		{		
			for(int i = 0; i < tablaARellenar.length; i++)
			{
				...
				{
					tablaFinal[i][j] = tablaARellenar[i][j];
				}
			}
			haySolucion = true; // hemos encontrado la solucion buscada
			return; // cortamos la ejecucions
		} else
		{
		...
		try
		{
			...
			while(linea != null)
			{
				...
				if(cont == 0)
				{
					...
					tablaFinal = new String[tamTablaRellenar][tamTablaRellenar];
				} else
				{
...
//										System.out.println(argumentos[i]);
	...
	public static void main(String[] args)
	{
		...
		System.out.println("\nTiempo en milisegundos: " + (after-before));
			...
			} else // estamos en fila normal...
			{
				...
				for(int j = 0; j < tablaTotalCargada[0].length; j++) // procesamos cada token de la fila i...
				{
					...
					} else // token normal (operador o numero)...
					{
						...
						{
							cadena += "\t" + tablaFinal[indiceFilaTablaRellenada][indiceColumnaTablaRellenada];
